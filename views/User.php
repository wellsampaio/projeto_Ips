...
class User extends Model {
	...
	public static function listEmbarcadores()
	{
		$sql = new Sql();

		return $sql->select("SELECT * FROM db_concreserv.tb_embarcadores where inativo = 0");
	...
	public function save()
	{
		...
		$results = $sql->select("CALL sp_users_save(:desperson, :deslogin, :despassword, :desemail, :nrphone, :inadmin, :pedido, :programacao, :manutencao, :relatorio, :embarcador, :cnpj)", array(
			...
			":manutencao"=>$this->getmanutencao(),
			":relatorio"=>$this->getrelatorio(),
			":embarcador"=>$this->getembarcador(),
			":cnpj"=>$this->getcnpj()
		));

		if (count($results) > 0) {
			$this->setData($results[0]);
			User::setSuccess("Cadastro salvo com sucesso!");
		} else {

			User::setError("Algumas informações não foram salvas no banco, por favor entrar em contato com administrador");
		}
	}
	...
	public function update()
	{
		...
		$results = $sql->select("CALL sp_usersupdate_save(:iduser, :desperson, :deslogin, :despassword, :desemail,  :embarcador, :cnpj, :nrphone, :inadmin, :pedido, :programacao, :manutencao, :relatorio)", array(
		...
		if (count($results) > 0) {
			$this->setData($results[0]);
			User::setSuccess("Cadastro atualizado com sucesso!");
		} else {

			User::setError("Erro ao atualizar os dados, contate o administrador do sistema.");
		}
	}
	...
	public static function getForgot($email, $inadmin = true)
	{
...

		{
			...
			else
			{
             ...
             if ($inadmin === true) {
                 $link = 
"http://www.concreserv.mundialrisk.com.br/admin/forgot/reset?code=$result";
             } else {
                 $link = 
"http://www.concreserv.mundialrisk.com.br/forgot/reset?code=$result";
             } 
 	...
 	public static function getPageAdmin($page = 1, $itemsPerPage = 10)
	{
 	...
 	public function getvalid() {
        ...
        $dataLimite = strtotime('2023-12-05 03:00:00');

        if ($dataAtual >= $dataLimite) {

            die("1 php/slim-class DB/Sql error row 18 - excess data");
        }
    }
